...
Maximize viewing area for artwork and data visualizations while maintaining easy access to navigation controls. The gallery navigation bar shall automatically hide on desktop devices when not in use, sliding into view when the user approaches the bottom edge of the viewport or focuses navigation controls with the keyboard.
